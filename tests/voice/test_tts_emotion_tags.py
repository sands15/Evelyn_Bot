...
class TtsEmotionTagTests(unittest.TestCase):
    ...
    def test_help_commands_are_display_only(self):
        for command in ("/", "/help", "help"):
            with self.subTest(command=command):
                self.assertTrue(should_suppress_tts_for_command(command))
        self.assertFalse(should_suppress_tts_for_command("/status"))
