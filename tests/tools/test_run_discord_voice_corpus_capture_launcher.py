...
"""
    ...
    def test_bounds_capture_validates_ten_wavs_and_restores_host_state(self) -> None:
        ...
        self.assertIn("[string]$StatusChannelId", source)
        self.assertIn("'--status-channel-id', $StatusChannelId", source)
        self.assertIn("'status_channel_unavailable'", source)
        self.assertIn("'status_send_failed'", source)
        self.assertIn("'guided_timing_invalid'", source)
