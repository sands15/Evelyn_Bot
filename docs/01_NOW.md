...
- validation GET은 현재 consent를 반환하고 local mic capture·speaker verification probe/enrollment·Opus/STT startup·LLM warmup body·Control Page welcome LLM non-200 body·Bridge·TTS warmup/control·Control Page server-start·router/tool-router, Discord autonomy-start·playback trace·voice connect retry·voice validation observer·turn-trace sink/fallback·voice last-channel state save·Minecraft snapshot, cognitive refresh·memory mirror·summary·proactive question promotion·vault maintenance·self-identity queue와 Main/Fast failed-tool·vision metrics/watch는 예외 원문을 가린다. Turn-trace의 일반 관측 예외는 원래 turn control flow로 전파하지 않는다. Runtime Error 관측은 Fast Control continuity를 포함하고 기록된 예외와 필수 service probe 장애를 구분한다. Optional payload-less 실패는 desired-state 부재로 health에만 남으며 backend 반영은 재시작 대기다.
  Discord 채널 이동은 guild lifecycle lock 아래 listener stop·세대 증가·retry/task 취소 요청·queue reset→guild voice TurnScope/TTS 정지→move·target 확인→connected listen 순서다. connect·move 중 ingress를 닫고, warmup은 lock 밖에서 실행해 새 channel event의 정리를 막지 않는다. event는 관측한 exact client/channel만 정리하며 binding을 ingress·STT 후·dispatch까지 재검증한다. 변경 직결 64개·voice 670개(skip 5)·core 798개(skip 1)와 최신 전체 3,333개(skip 22)가 통과했고, 실제 two-channel/live audio는 미검증이다.
...
- Control Page 강제 검색은 새 text turn/TurnScope와 두 짧은 critical section으로 이전 일반 턴과 stale sink를 막고, 일반·검색 TTS는 task 완료까지 scope를 유지한다.
  Discord 자율시작의 실제 생성·cleanup·start 실패와 자율정지 cleanup 실패는 type-only Runtime Error로 남고 관측기·성공 응답 실패는 명령 상태를 바꾸거나 cleanup 실패로 오분류되지 않으며 취소는 grant를 회수한 뒤 재전파한다. normal text outer failure도 shared Discord status에 fixed code/type/count 기록을 시도하며 observer 실패는 고정 응답·continuity를 막지 않는다. 대화형 자율 후속은 configured channel/thread 경계 안의 최신 active canonical Discord text session과 exact message/channel/last-active snapshot에 observation부터 결박하고 voice/default·만료·변경 target은 제외한다. busy/no-target은 question·send·ping·history·commit·memory 없이 transient blocked이며, send부터 exact recipient commit까지 기존 reply→session lock을 쓴다. cognitive refresh는 exact session을 잡은 뒤 reply slot을 한 successor text turn에 넘기고 prefixed command도 같은 state lock을 사용한다. `send_discord_text` await 자체의 일반 실패는 shared Discord Runtime Errors에 fixed `autonomy_followup_send_failed`/type/count 기록을 시도하고 원예외를 재전파하며, send 정상 반환 뒤에는 900초 fence와 verified 전달/cursor를 유지한다. 취소·memory-integrity 신호는 재전파한다. 실제 Discord-channel playback은 audio/finalization 뒤 exact target에 visible text를 한 번 시도하고 non-null memory exposure의 send용 deletion lease를 다시 획득한다. Windows Local I/O Bridge/Control Page `/mic on`은 기존 Fast 경로로 유지하고 `LocalControlVoiceClient(channel=None)`는 이 projection 대상에서 제외한다. 음성 completion은 선택적 memory/cognitive/search보다 assistant history·active state·room owner 반영과 continuity commit 시도를 먼저 실행하며, durable receipt가 나온 경우 후속 선택 작업 실패가 checkpoint를 되돌리지 않는다. 음성 text-send 실패는 shared Discord Runtime Errors와 voice pipeline에 fixed code/type 기록을 각각 시도하며 rollback·retry하지 않는다. 변경 직결 72개·autonomy 인접 132개·Discord I/O 139개와 최신 전체 3,331개(skip 22)가 통과했고 live 서비스는 기동하지 않았다.
