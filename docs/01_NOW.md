...
- 2026-08-09 Mindcraft history는 bounded process-local·no-mount이며 Codex는 off다.
  ...
  recovery step은 exact history snapshot의 process-local one-shot issuance만 소비하고, 손상 world-effect policy는 validation 예외나 false-ready 대신 fixed blocker로 닫힌다.
...
- validation GET은 현재 consent를 반환하고 local mic capture·Opus/STT startup·LLM warmup body·Bridge·TTS warmup/control·router, Discord playback trace·Minecraft snapshot, memory mirror·summary·self-identity queue와 Main/Fast failed-tool·vision metrics/watch는 예외 원문을 가린다. Runtime Health probe도 timeout을 지킨다.
...
- CI-equivalent 전체 3,229개(skip 22), Local Bridge continuity 74개, Mindcraft 56개,
