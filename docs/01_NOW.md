...
- Local Voice는 단일 capture owner, durable claim·consent fence로 재시작 경쟁을 막고, TTS 중 첫 threshold 후보의 playback generation을 flush까지 보존한다. 같은 generation이 해제된 tail은 새 owner·validation이 없고 선두 `이블린`이 확인될 때만 기존 admission으로 보낸다. `/mic on`은 오른쪽 drawer의 검증·청취 동의 버튼을 표시·포커스한다.
  Local Bridge 재생 실패는 user-only로 잇는다. Discord voice는 수락한 user-only turn을 LLM/TTS 전에 durable commit하고, 성공 시 exact tail에 assistant/receipt만 붙이며 실패·취소와 receipt 직후 hard-exit은 user-only 문맥으로 복구한다. Owner continuity restore는 checkpoint/row effective age가 TTL을 넘거나 row activity age를 증명할 수 없으면 그 history를 복구하지 않아 다른 session flush가 수명을 늘리지 못한다.
- validation GET은 현재 consent를 반환하고 local mic capture·speaker verification probe/enrollment·Opus/STT startup·LLM warmup body·Control Page welcome LLM non-200 body·Bridge·TTS warmup/control·Control Page server-start·router/tool-router, Discord autonomy-start·playback trace·voice connect retry·voice validation observer·turn-trace sink/fallback·voice last-channel state save·Minecraft snapshot, cognitive refresh·memory mirror·summary·proactive question promotion·vault maintenance·self-identity queue와 Main/Fast failed-tool·vision metrics/watch는 예외 원문을 가린다. Turn-trace의 일반 관측 예외는 원래 turn control flow로 전파하지 않는다. Runtime Error 관측은 Fast Control continuity를 포함하고 기록된 예외와 필수 service probe 장애를 구분한다. Optional payload-less 실패는 desired-state 부재로 health에만 남으며 backend 반영은 재시작 대기다.
  Discord 채널 이동은 guild lifecycle lock 아래 listener stop·세대 증가·retry/task 취소 요청·queue reset→guild voice TurnScope/TTS 정지→move·target 확인→connected listen 순서다. connect·move 중 ingress를 닫고, warmup은 lock 밖에서 실행해 새 channel event의 정리를 막지 않는다. event는 관측한 exact client/channel만 정리하며 binding을 ingress·STT 후·dispatch까지 재검증한다. 실제 two-channel live는 미검증이다.
