...
- legacy layered memory는 deletion-current lineage가 생길 때까지 stored summary,
  ...
  제외한다. 현재 automatic daily writer는 scope label을 받아도 `daily/guild-<id>/<date>.md` 하나에
  합치고 owner binding을 durable metadata로 보존하지 않는다. 따라서 같은 guild의 exact path,
  reset scope, parent와 body hash도 principal 노출 권한을 증명하지 않는다. 2026-09-02 격리 검증은
  deterministic·semantic child가 실제 생성된 뒤 같은 guild 조회에서도 계속 보류됨을 고정했다.
  retrieval cache v3와 hot-context recall policy marker가 과거 cache 재사용을 막는다.
  provenance-bearing `open_questions.jsonl` 저장·감사, explicit user/system note와 현재 모델 답변의
  명시적 질문은 그대로 유지한다.
