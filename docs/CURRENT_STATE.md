...
- `main.py` 분해를 재개했다. 음성 hot path의 ingress/audio filtering, wake probe/환경음 조기 차단,
  ...
  short transcript/final wake session gate도 분리했으며 reply context/dependency 조립은 아직 남아 있다.
...
검증 시각: 2026-07-15 19:51 KST
검증한 코드 기준점: `refactor/main-py-decomposition-2026-07-15` 여섯 번째 voice session gate 분리 작업 트리

- `pip check`: 통과
- `docker compose config --quiet`: 통과(검증용 Discord token 사용)
- Python `compileall`: 통과
- 활성 Live2D/boot JavaScript `node --check`: 통과
- 전체 unittest: 999개 통과, 실패 0, 오류 0, 건너뜀 0
