...
- `main.py` 분해를 재개했다. 음성 hot path의 ingress/audio filtering, wake probe/환경음 조기 차단,
  ...
  runtime dependency builder 군과 Discord 단일 TTS 진입점 등은 아직 `main.py`에 남아 있다.
...
검증 시각: 2026-07-16 00:29 KST
검증한 코드 기준점: `refactor/main-py-decomposition-2026-07-15` 스무 번째 router JSON HTTP 분리 작업 트리

- `pip check`: 통과
- `docker compose config --quiet`: 통과(검증용 Discord token 사용)
- Python `compileall`: 통과
- 활성 Live2D/boot JavaScript `node --check`: 통과
- 전체 unittest: 일반 모드 1,075개 통과(실제 `main.py` opt-in 1개 의도적 건너뜀), 엄격 모드 1,075개 통과
