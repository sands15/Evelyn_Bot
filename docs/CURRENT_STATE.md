...
- `main.py` 분해를 재개했다. 음성 hot path의 ingress/audio filtering, wake probe/환경음 조기 차단,
  ...
  short transcript/final wake session gate와 reply context 구간은 아직 남아 있다.
...
검증 시각: 2026-07-15 19:40 KST
검증한 코드 기준점: `refactor/main-py-decomposition-2026-07-15` 다섯 번째 transcript/barge-in 분리 작업 트리

- `pip check`: 통과
- `docker compose config --quiet`: 통과(검증용 Discord token 사용)
- Python `compileall`: 통과
- 활성 Live2D/boot JavaScript `node --check`: 통과
- 전체 unittest: 992개 통과, 실패 0, 오류 0, 건너뜀 0
