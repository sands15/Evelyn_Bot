...
- `main.py` 분해를 재개했다. 음성 hot path의 ingress/audio filtering, wake probe/환경음 조기 차단,
  TTS interrupt/input suppression gate, partial/full STT 실행을 런타임 모듈로 옮겼다.
  transcript/barge-in merge, session gate, reply context 구간은 아직 남아 있다.
...
검증 시각: 2026-07-15 19:26 KST
검증한 코드 기준점: `refactor/main-py-decomposition-2026-07-15` 네 번째 partial/full STT 분리 작업 트리

- `pip check`: 통과
- `docker compose config --quiet`: 통과(검증용 Discord token 사용)
- Python `compileall`: 통과
- 활성 Live2D/boot JavaScript `node --check`: 통과
- 전체 unittest: 985개 통과, 실패 0, 오류 0, 건너뜀 0
