const DEBUG_ALLOW_ALL_SLASH_COMMANDS = (process.env.VOYAGER_DEBUG_COMMANDS || "false").toLowerCase() === "true";
const SLASH_COMMAND_WHITELIST = new Set(["clear", "kill"]);

function isAllowedSlashCommand(text) {
    if (!text.startsWith("/")) {
        return false;
    }
    if (DEBUG_ALLOW_ALL_SLASH_COMMANDS) {
        return true;
    }
    const commandName = text.slice(1).trim().split(/\s+/, 1)[0]?.toLowerCase() || "";
    return SLASH_COMMAND_WHITELIST.has(commandName);
}
...
function inject(bot) {
    ...
    bot.chat = (message) => {
        ...
        // action_count.chat++;
        bot.emit("chatEvent", "bot", text);
        if (isAllowedSlashCommand(text)) {
            bot._chat(text);
        }
    };
