async function haveEightIronOreIntoIronIngots(bot) {
  ...
  if (countItem(oreName) < neededIngots) {
    ...
    const nearbyOre = bot.findBlocks({
      matching: block => block?.name === oreName,
      maxDistance: 32,
      count: missingOre
    });
    if (nearbyOre.length > 0) {
      await mineBlock(bot, oreName, Math.min(nearbyOre.length, missingOre));
    }
    if (countItem(oreName) < neededIngots) {
  ...
  if (countItem(oreName) < neededIngots) throw new Error("NOT_ENOUGH_IRON_ORE_FOR_SMELTING");
