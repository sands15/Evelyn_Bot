async function smelt5RawIronIntoIronIngotsTask(bot) {
  ...
  const ensureSmelting = async () => {
    ...
    // Choose fuel that we have at least neededRaw of
    ...
    // smeltItem helper is available (uses/opens furnace internally)
    await smeltItem(bot, "raw_iron", fuelName, neededRaw);

    // Contract: inventory after should have produced enough ingots,
    // and raw iron decreased by at least neededRaw.
    ...
    if (produced < neededRaw) throw new Error("SMELTING_FAILED_TO_CONVERT_EXPECTED_AMOUNT");
