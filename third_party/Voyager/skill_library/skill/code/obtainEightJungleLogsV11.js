async function obtainEightJungleLogs(bot) {
  ...
  const currentCount = bot.inventory.count(jungleLog.id, null);
  if (currentCount >= 8) {
    return;
  }
  ...
  const remaining = 8 - bot.inventory.count(jungleLog.id, null);
  if (remaining > 0) {
    await mineBlock(bot, "jungle_log", remaining);
  }
  if (bot.inventory.count(jungleLog.id, null) < 8) {
